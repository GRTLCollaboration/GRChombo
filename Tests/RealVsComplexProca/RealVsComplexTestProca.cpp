...
int main()
{
    ...
    const int N_GRID = 64;
    const int NUM_REAL_VARS = 1 + c_Zvec - c_Avec0;
...
#pragma omp parallel for
    for (int zz = -3; zz < N_GRID + 3; ++zz)
    {
        ...
        for (int yy = -3; yy < N_GRID + 3; ++yy)
        {
            ...
            for (int xx = -3; xx < N_GRID + 3; ++xx)
            {
                ...
                // the matter vars
                in_fab(iv, c_Avec0) = 2.73578 + 0.36898 * x + 0.14348 * x * x +
                                      3.33487 * x * y * y * y +
                                      2.99469 * y * z + 0.20515 * z * z +
                                      3.88385 * z * z * z * z;
                in_fab(iv, c_Avec0_Re) = in_fab(iv, c_Avec0);
                in_fab(iv, c_Avec0_Im) = in_fab(iv, c_Avec0);
                in_fab(iv, c_Avec1) = 1.73578 + 2.36898 * x + 0.54348 * x * x +
                                      3.33487 * x * y * y * y +
                                      1.99469 * y * z + 1.20015 * z * z +
                                      4.88385 * z * z * z * z;
                in_fab(iv, c_Avec1_Re) = in_fab(iv, c_Avec1);
                in_fab(iv, c_Avec1_Im) = in_fab(iv, c_Avec1);
                in_fab(iv, c_Avec2) = 5.73578 + 1.36898 * x + 0.54348 * x * x +
                                      3.33487 * x * y * y * y +
                                      8.99469 * y * z + 2.20015 * z * z +
                                      1.88385 * z * z * z * z;
                in_fab(iv, c_Avec2_Re) = in_fab(iv, c_Avec2);
                in_fab(iv, c_Avec2_Im) = in_fab(iv, c_Avec2);
                in_fab(iv, c_Avec3) = 0.73578 + 5.36898 * x + 0.54348 * x * x +
                                      6.33487 * x * y * y * y +
                                      0.99469 * y * z + 4.20015 * z * z +
                                      2.88385 * z * z * z * z;
                in_fab(iv, c_Avec3_Re) = in_fab(iv, c_Avec3);
                in_fab(iv, c_Avec3_Im) = in_fab(iv, c_Avec3);
                // and for Evec
                in_fab(iv, c_Evec1) = 1.73578 + 2.36898 * x + 0.54348 * x * x +
                                      3.33487 * x * y * y * y +
                                      1.99469 * y * z + 1.20015 * z * z +
                                      4.88385 * z * z * z * z;
                in_fab(iv, c_Evec1_Re) = in_fab(iv, c_Evec1);
                in_fab(iv, c_Evec1_Im) = in_fab(iv, c_Evec1);
                in_fab(iv, c_Evec2) = 5.73578 + 1.36898 * x + 0.54348 * x * x +
                                      3.33487 * x * y * y * y +
                                      8.99469 * y * z + 2.20015 * z * z +
                                      1.88385 * z * z * z * z;
                in_fab(iv, c_Evec2_Re) = in_fab(iv, c_Evec2);
                in_fab(iv, c_Evec2_Im) = in_fab(iv, c_Evec2);
                in_fab(iv, c_Evec3) = 0.73578 + 5.36898 * x + 0.54348 * x * x +
                                      6.33487 * x * y * y * y +
                                      0.99469 * y * z + 4.20015 * z * z +
                                      2.88385 * z * z * z * z;
                in_fab(iv, c_Evec3_Re) = in_fab(iv, c_Evec3);
                in_fab(iv, c_Evec3_Im) = in_fab(iv, c_Evec3);
                // and Zvec
                in_fab(iv, c_Zvec) = 1.73578 + 1.36898 * x + 0.34348 * x * x +
                                     1.33487 * x * y * y * y + 1.99469 * y * z +
                                     0.20015 * z * z + 1.88385 * z * z * z * z;
                in_fab(iv, c_Zvec_Re) = in_fab(iv, c_Zvec);
                in_fab(iv, c_Zvec_Im) = in_fab(iv, c_Zvec);
            }
        }
    }
    ...
    params.eta = 1.82;
    double sigma = 0.3;
    double vector_mass = 1.0;
    double vector_damping = 0.5;

    ProcaField proca_field(vector_mass, vector_damping);
    ComplexProcaField complex_proca_field(vector_mass, vector_damping);
    BoxLoops::loop(MatterCCZ4<ProcaField>(proca_field, params, dx, sigma),
                   in_fab, out_fab);
    BoxLoops::loop(
        MatterCCZ4<ComplexProcaField>(complex_proca_field, params, dx, sigma),
        in_fab, out_fab);

    // Add the EM tensor rho, S and Si - use in_fab for real, out_fab for im
    BoxLoops::loop(EMTensor<ProcaField>(proca_field, dx), in_fab, in_fab);
    BoxLoops::loop(EMTensor<ComplexProcaField>(complex_proca_field, dx), in_fab,
                   out_fab);
...
#pragma omp parallel for
    for (int zz = 0; zz < N_GRID; ++zz)
    {
        ...
        for (int yy = 0; yy < N_GRID; ++yy)
        {
            ...
            for (int xx = 0; xx < N_GRID; ++xx)
            {
                ...
                for (int i = c_Avec0; i < c_Avec0 + NUM_REAL_VARS; ++i)
                {
    ...
    if (failed == 0)
        std::cout << "RealVsComplexProca test passed..." << std::endl;
    else
        std::cout << "RealVsComplexProca test failed..." << std::endl;
