...
class SphericalExtraction : public SurfaceExtraction
{
  public:
    ...
    ~SphericalExtraction() { delete m_geom_ptr; }
    ...
    std::pair<std::vector<double>, std::vector<double>> integrate_mode(
        ...
        std::function<std::pair<double, double>(std::vector<double>, double,
            ...
            a_function)
    {
        auto integrand_re = [center = m_center, geom_ptr = m_geom_ptr, es, el,
                             em, &a_function](std::vector<double> a_data_here,
                                              double r, double theta,
                                              double phi) {
            // note that spin_Y_lm requires the coordinates with the center
            // at the origin
            double x = geom_ptr->cartesian_coord(0, r, theta, phi) - center[0];
            double y = geom_ptr->cartesian_coord(1, r, theta, phi) - center[1];
            double z = geom_ptr->cartesian_coord(2, r, theta, phi) - center[2];
        ...
        auto integrand_im = [center = m_center, geom_ptr = m_geom_ptr, es, el,
                             em, &a_function](std::vector<double> a_data_here,
                                              double r, double theta,
                                              double phi) {
            // note that spin_Y_lm requires the coordinates with the center
